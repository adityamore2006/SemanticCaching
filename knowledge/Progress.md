...
- **One-command reset** (`scripts/reset_cache.py` + `POST /admin/reset`): restores the cache to exactly the canonical 67. Reset lives in the service because the in-memory index is what actually needs clearing and only the process holds it; purging DynamoDB from outside leaves the running index serving entries that no longer exist in storage. It clears the store, deletes the on-disk snapshot (skipping that step is what silently lets a restart undo the reset), rebuilds, and re-seeds. Guarded by `RESET_TOKEN` and **disabled entirely (404) when unset**, since the API is reachable from anywhere. The script verifies afterwards rather than trusting the response.
- **`CacheStore.clear()`** on the contract, skipping reserved `__`-prefixed rows so a demo reset never quietly wipes the daily spend counter.
- **`scripts/purge_cache.py`:** clears stub entries, which otherwise survive the switch to real Bedrock and keep being served as hits. `scripts/seed_cache.py` was deleted: redundant once the service auto-seeds, and broken once a miss with no model returns 501 instead of caching.
